...
### Added
- **GNSS-denied navigation** — `gnss_denied_nav.py` visual odometry + BlueOS optical flow for flight without satellites
- **Supply chain policy** — avoid Chinese electronics; camera selection updated to Gremsy (Vietnam) + FLIR (USA)
- **Satellite reference matching** — CNN cross-view (EfficientNet-B2, Colab A100): median 9.6m error, 70% under 25m. ORB baseline: 0.5m.
